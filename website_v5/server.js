...
// NOTE:
// This project used to start a second server on port 3000 just to print the URL.
// That causes confusion when you also set PORT (e.g. 3100) and makes it look like two sites.
// Keep only ONE server (the one at the bottom that uses PORT).
// app.listen(3000, () => console.log('http://localhost:3000'));

// Default to 3100 so user doesn't need to set $env:PORT every time.
...
app.get('/whoami', (req, res) => {
  res.json({ from: 'website_v5', dir: __dirname, now: new Date().toISOString() });
});
...
app.listen(PORT, () => {
  ...
  // PowerShell: $env:OPEN_BROWSER="false"
  if (String(process.env.OPEN_BROWSER || '').toLowerCase() !== 'false') {
    // Small delay helps ensure the server is ready before the browser hits it.
    setTimeout(() => openDefaultBrowser(url), 150);
  }
});